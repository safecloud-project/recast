...
def stop_minio(provider_conf, ssh_port=22):
    """
    ...
    command = ("ssh -o StrictHostKeyChecking=no -p {:d} {:s} " +
               "docker ps -aq | xargs --no-run-if-empty docker rm -fv").format(ssh_port,
                                                                               minio_host)
    ret = subprocess.call(command.split())
    return ret

def stop_redis(provider_conf, ssh_port=22):
    """
...
def start(configuration):
    ...
    for provider in providers:
        ...
        if provider_type == "s3":
            stop_minio(providers[provider])
        elif provider_type == "redis":
            stop_redis(providers[provider])
        else:
