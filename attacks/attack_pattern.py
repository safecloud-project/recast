...
def kill_random_docs(NUM_DOCS, percentage, blocks_per_doc, p, KEYS):
    " Output the list of lost blocks "
    # percentage: float in [0,1] ex. 0.4 = 40%
    ...
    for n in down_docs:
        ...
        LOSTBLOCKS = LOSTBLOCKS + [KEYS[n]+"-%s"%i for i in indices]
    
    return LOSTBLOCKS

def kill_random(NUM_DOCS, percentage, p, KEYS):
    " Output the list of lost blocks "
    # percentage: float in [0,1] ex. 0.4 = 40%
    # blocks_per_doc : how many pointer blocks per doc to erase
    # w is the canvas
    ...
    while count < num_down_blocks:
        ...
        LOSTBLOCKS[count] = KEYS[doc_idx]+"-%s"%block_idx
        count = count + 1
    ...
    while len(LOSTBLOCKS) < num_down_blocks:
        doc_idx = random.choice(range(NUM_DOCS))
        block_idx = str(random.choice(range(p))).zfill(NDIGIT) 
        LOSTBLOCKS.add( KEYS[doc_idx]+"-%s"%block_idx )
        
    return list(LOSTBLOCKS)

def draw_random(heuristic, NUM_DOCS, percentage, blocks_per_doc, p, EDGES, w, KEYS, BLOCKSTONODES = None):
    if (heuristic == "KillNodes"):
        LOSTBLOCKS = kill_random_nodes(BLOCKSTONODES, percentage)
    elif (heuristic == "KillDocs"):
        LOSTBLOCKS = kill_random_docs(NUM_DOCS, percentage, blocks_per_doc, p, KEYS)
    elif (heuristic == "KillRand"):
        LOSTBLOCKS = kill_random(NUM_DOCS, percentage, p, KEYS)
    else:
...
def mymin(docsset, KEYS):
    # Return the minimal index of the documents in the set docsset
    indices = [ KEYS.index( docsset[i] ) for i in range(len(docsset)) ]
    return min(indices)

def mymax(docsset, KEYS):
    # Return the maximal index of the documents in the set docsset
    indices = [ KEYS.index( docsset[i] ) for i in range(len(docsset)) ]
    return max(indices)

def CreepingAttack(C, b, POINTERDICT, KEYS):
    C1 = list(set(C + documents(b, POINTERDICT,KEYS)))
    return mymax(C1,KEYS) - mymin(C1,KEYS)

def MinimumAttack(C, b, POINTERDICT, KEYS):
    return len( list(set(C+documents(b, POINTERDICT, KEYS))) )

def LeapingAttack(C, b, POINTERDICT, KEYS):
    N = documents(b, POINTERDICT, KEYS)
    for c in C:
        if (c in N):
            N.remove(c)
...
def TailoredAttack(C, B, b, POINTERDICT, t, e, p, K, KEYS):
    import math
    cost = 0
    B1 = B + [b]
    C1 = list(set(C + documents(b, POINTERDICT,KEYS)))
...
def greedy_attack(heuristic, target_doc, t, e, p, POINTERDICT, NUM_DOCS, KEYS):
    ...
    while (R != []) :
        r = KEYS[mymin(R,KEYS)] # chronological order
        while (count_blocks(r, B, p, POINTERDICT) < (e+1)):
            ...
            for b in range(p):
                ...
                if (theblock not in B):
                    if (heuristic == "MinimumAttack"):
                        score[theblock] = MinimumAttack(C, theblock , POINTERDICT, KEYS)
                    elif (heuristic == "LeapingAttack"):
                        score[theblock] = LeapingAttack(C, theblock , POINTERDICT, KEYS)
                    elif (heuristic == "CreepingAttack"):
                        score[theblock] = CreepingAttack(C, theblock , POINTERDICT, KEYS)
                    elif (heuristic == "TailoredAttack"):
                        score[theblock] = TailoredAttack(C, B, theblock, POINTERDICT, t, e, p, NUM_DOCS, KEYS)
                    else:
            ...
            B = B + [b]
            docs = documents(b, POINTERDICT, KEYS)
            docsC = docs[:]
            for d in C:
                if (d in docsC):
                    docsC.remove(d)
...
def draw_greedy(heuristic, w, EDGES, target_doc, t, e, p, POINTERDICT, NUM_DOCS, KEYS):
    ...
    LOSTBLOCKS = greedy_attack(heuristic, target_doc, t, e, p, POINTERDICT, NUM_DOCS, KEYS)
    for b in LOSTBLOCKS: # paint parity blocks
        if (target_doc in b):
            w.itemconfig(b, fill = "red") # target doc
        else:
