...
for i in $(seq 10); do
  for BLOCK_SIZE in "$@"; do
    ...
    docker run -it --rm -v "${PWD}/xpdata":/opt/xpdata client /bin/bash -c "cd /opt/xpdata/macrobench/${EC_TYPE}/${i} && ls /home && /ab_playcloud.sh 1000 50 ${BLOCK_SIZE} ${PROXY_PORT_3000_TCP_ADDR}:${PROXY_PORT_3000_TCP_PORT} > stdout.txt"
  done
done
