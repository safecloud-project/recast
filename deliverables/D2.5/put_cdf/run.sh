...
main() {
	...
	for directory in ${directories[@]}; do
		...
		for run in $(seq 1 "${RUNS}"); do
			...
			ssh -o "StrictHostKeyChecking no" "${MAIN_HOST}" "cd playcloud && docker-compose stop && docker-compose rm -fv && sudo find volumes/ -type f -delete && docker-compose build && ./deploy_redis.sh && docker-compose up -d && sleep 12"
